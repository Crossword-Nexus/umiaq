...
fn is_valid_binding(val: &str, constraints: &VarConstraint, bindings: &Bindings) -> bool {
    ...
    if val.len() < constraints.bounds.li
        || constraints.bounds.ui.is_some_and(|max_len| val.len() > max_len)
    {
        return false;
    }
            ...
            FormPart::Var(var_name) | FormPart::RevVar(var_name) => {
                if let Some(bound_val) = self.bindings.get(*var_name) {
                    ...
                    let min_len = self
                        .constraints
                        .get(*var_name)
                        .map_or(VarConstraint::DEFAULT_MIN, |vc| vc.bounds.li);
                    let max_len_cfg = self
                        .constraints
                        .get(*var_name)
                        .and_then(|vc| vc.bounds.ui)
                        .unwrap_or(chars.len());
