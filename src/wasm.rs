...
pub fn get_debug_info(
    ...
    let _ = writeln!(&mut report, "Built: {}", env!("BUILD_TIMESTAMP"));
    let _ = writeln!(&mut report, "Commit: {}", env!("GIT_HASH"));
    let _ = writeln!(&mut report, "Report generated: {}", js_sys::Date::new_0().to_iso_string().as_string().unwrap_or_else(|| "unknown".to_string()));
